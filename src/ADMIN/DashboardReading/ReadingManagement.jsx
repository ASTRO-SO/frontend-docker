...
const fetchAstrologyReadingsFromDatabase = async () => {
  try {
    const response = await fetch('https://backend-docker-production-c584.up.railway.app/api/astrology/readings', {
      credentials: 'include'
    });
    if (!response.ok) {
...
const fetchNumerologyReadingsFromDatabase = async () => {
  try {
    const response = await fetch('https://backend-docker-production-c584.up.railway.app/api/numerology/readings', {
      credentials: 'include'
    });
    if (!response.ok) {
...
const fetchUserDetails = async (phone) => {
  try {
    const response = await fetch(`https://backend-docker-production-c584.up.railway.app/api/users/phone/${phone}`, {
      credentials: 'include'
    });
    if (!response.ok) {
      return null;
